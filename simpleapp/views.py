...
def signup():
  ...
  if request.method == "POST":
    if account.validate_on_submit():
      newuser = User(username = account.username.data, password = account.password.data, email="abc@gmail.com", date_created = db.Column(db.DateTime, default=datetime.now()))
...
def wtform():
  ...
  if request.method == "POST":
    if form.validate_on_submit():
      ...
      newpost = Post(name = form.name.data, description = form.description.data, tdate = form.tdate.data, completed = form.completed.data, user_id = current_user.id)
      ...
      for tag in tags:
        ...
        if not gotTagByThisUser: #if current user does not have this tag, create new tag under this user
          ...
          newpost.tags.append(gotTagByThisUser) #add the existing tag to current newtodo
