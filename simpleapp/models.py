...
class Profile(db.Model):
  ...
  user_id = db.Column(db.Integer, db.ForeignKey('user.id')) #ONE TO ONE relationship with User
...
class User(db.Model, UserMixin):
  ...
  posts = db.relationship("Post", backref = "user", lazy='joined') #ONE TO MANY relship
  ...
  profile = db.relationship("Profile", backref = 'user',  lazy = "joined", uselist=False) #ONE TO ONE relationship with Profile
...
class Post(db.Model):
  ...
  tags = db.relationship("Tag", secondary="categories", backref=db.backref("posts", lazy = "dynamic"))
...
class Tag(db.Model):
  id = db.Column(db.Integer, primary_key = True)
  name = db.Column(db.String(256))
  user_id = db.Column(db.Integer, db.ForeignKey("user.id")) #Which user did this tag belong to?
...
def insert_dummy_data(db):
  ...
  post1 = Post(name="Prepare CLE", description = "Print worksheets", tdate = datetime.now(), completed = False, user_id = admin.id)

  db.session.add(post1)

  #Adding Todo2, without specifying which owner
  post2 = Post(name="Prepare CEP notes", description = "Email teachers for project specifications", tdate = datetime.now() + timedelta(days=14), completed = False)
  ...
  post3 = Post(name="WA4", description = "Issue WA4 requirements", tdate = datetime.now() + timedelta(days=7), completed = False)

  #Adding Todo4 and not specifying who it belongs to
  post4 = Post(name="CCA records", description = "Update CCA records", tdate = datetime.now() + timedelta(days=7), completed = False)
